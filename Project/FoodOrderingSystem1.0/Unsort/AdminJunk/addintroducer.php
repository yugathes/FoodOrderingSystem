...
<head>
  ...
  <title>Southern Finance | Add Introducer</title>
  <!-- Favicon -->
          ...
          <ul class="navbar-nav">
            ...
            <li class="nav-item">
              <a class="nav-link" href="addintroducer.php">
                ...
                <span class="nav-link-text font-weight-bold text-success">Add Introducers</span>
              </a>
            </li>
            <li class="nav-item">
            ...
            <div class="card-header bg-transparent">
              <div class="row align-items-center">
                <div class="col">
                  ...
                  <h5 class="h2 text-white mb-0">Add New Introducer</h5>
                </div>
                <div class="col">
                ...
                <!-- Chart wrapper -->
                <div class="card-body">
                <form action="addintroducer.php" method="POST" enctype='multipart/form-data'>
...
<div class="col-xl-3">
  <div class="form-group">
    ...
    <input type="text" class="form-control text-dark" id="exampleFormControlInput1" name="iName">
  </div>

  <div class="form-group">
    <label for="exampleFormControlSelect1" class="h5 text-white mb-2" >Phone No. *</label>
    <input type="text" class="form-control text-dark" id="exampleFormControlInput1" name="number">
  </div>
</div>
...
<div class="col-xl-3">
    ...
    <input type="password" class="form-control text-dark" id="exampleFormControlInput1" name="password">
  </div>
  <div class="form-group">
    <label for="exampleFormControlSelect1" class="h5 text-white mb-2" >Email *</label>
    <input type="email" class="form-control text-dark" id="exampleFormControlInput1" name="email">
  </div>
</div>
...
<div class="row">
  ...
  <div class="form-group">
    ...
    <input type="number" class="form-control" id="exampleFormControlInput1" name="icno">
  </div>
...
<div class="col-xl-3">
  <div class="form-group">
    ...
    <input type="file" class="form-control text-dark" id="exampleFormControlInput1" name="file">
  </div>
...
<div class="col-xl-3">
    ...
    <input type="text" class="form-control text-dark" id="exampleFormControlInput1" name="bno">
  </div>
    ...
    <input class="btn btn-primary" type="submit" name="addintro" value="Add">
</form>
...
<?php
  ...
  // receive all input values from the form
	...
	$icno = mysqli_real_escape_string($link, $_POST['icno']);
	$bname = mysqli_real_escape_string($link, $_POST['bname']);
	$bno = mysqli_real_escape_string($link, $_POST['bno']);
	
	if (empty($name)) { array_push($errors, "Full Name is required"); }
    if (empty($address)) { array_push($errors, "Address is required"); }
    if (empty($iname)) { array_push($errors, "Introducer Username is required"); }
    if (empty($number)) { array_push($errors, "Phone No is required"); }
    if (empty($password)) { array_push($errors, "Account Password is required"); }
    if (empty($email)) { array_push($errors, "Email is required"); }
    if (empty($icno)) { array_push($errors, "IC No is required"); }
    if (empty($bname)) { array_push($errors, "Bank Name is required"); }
    if (empty($bno)) { array_push($errors, " Bank Acc No is required"); }
    
    $intro_check_query = "SELECT * FROM introducer WHERE iName='$iname' LIMIT 1";
    $result = mysqli_query($link, $intro_check_query);
    $intro = mysqli_fetch_assoc($result);
    if ($intro) { // if user exists
    if ($intro['iName'] === $iname) {
      array_push($errors, "Introducer Name already exists");
    ...
    $destination = 'introImg/' . $filename;
    // get the file extension
    $extension = pathinfo($filename, PATHINFO_EXTENSION);

    // the physical file on a temporary uploads directory on the server
    $file = $_FILES['icimg']['intro'];
    ...
    if (!in_array($extension, ['jpeg', 'jpg', 'png'])) {
  	...
  	$query = "INSERT INTO introducer(usrID, name, address, iName, Hp, password, email, icno, icimg, bname, bno, tclient, tEarn, withdraw) 
  			  VALUES('$usrID', '$name', '$address', '$iname', '$number', '$password', '$email', '$icno', '$filename', '$bname', '$bno', '0', '0', '0')";
  	$result= mysqli_query($link, $query);
  	if (!$result)
	{
	    die("Error:".mysqli_error($link));
	   $fail = "Please Check Introducer Detaile.";
        echo "<script type='text/javascript'>alert('$fail');
	    document.location='addintroducer.php';
	    </script>"; 
	}
	else
	{
	    $sucess = "Introducer Added Sucessful.";
