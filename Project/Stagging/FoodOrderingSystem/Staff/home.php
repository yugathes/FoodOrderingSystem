...
<?php
	...
	$resultGetO = mysqli_query($link,"SELECT name FROM shop WHERE id = '".$category."'");
	$barisO= mysqli_fetch_array($resultGetO, MYSQLI_BOTH);
	$shopName = $barisO['name'];
	if(!$resultGet)
	{
			...
			<form class = "content" style="position:absolute;width: 40%;" action="../Admin/Update.php" name="EditForm" method="POST">
				...
				<?php	while($baris= mysqli_fetch_array($resultGet, MYSQLI_BOTH))	{?>
					<div class="input-group">
						...
						<input type="text" name="shop" value="<?php echo $shopName; ?>" disabled><br><br>
